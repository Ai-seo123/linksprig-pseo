...
class InternalLinker:
    def __init__(self, registered_pages=None):
        ...
        [
            {"post_type": "compare", "slug": "/compare/buzzstream-vs-petalrank", "title": "BuzzStream vs PetalRank", "entity": "BuzzStream"},
            {"post_type": "industry", "slug": "/industry/search-visibility-for-saas", "title": "Search Visibility for SaaS", "entity": "SaaS"},
            ...
        ]
        """
        ...
        for target_type in target_types:
            if target_type in static_destinations:
                ...
                if url not in used_urls:
                    candidates.append({"title": f"PetalRank {target_type.capitalize()}", "slug": url})
            else:
                for page in self.registered_pages:
                    url = page["slug"]
