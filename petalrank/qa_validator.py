...
class QAValidator:
    ...
    def validate_page(self, post_type, generated_data, competitor_data=None):
        """
        Validates the generated page against the PetalRank QA rules.
        Returns:
        ...
        # 6. Keyword Overlap (Keyword stuffing check - e.g. 'PetalRank' density > 6%)
        petalrank_count = plain_content.count("petalrank")
        total_words = len(plain_content.split())
        if total_words > 0:
            density = (petalrank_count / total_words) * 100
            if density > 6.0:
                reasons.append(f"Keyword stuffing: Brand term 'PetalRank' density is {density:.2f}%, exceeding the 6% limit.")

        # 7. Low Differentiation / Missing Required Phrases
        required_phrases = ["geo tracking", "ai search visibility", "search landscapes", "espial solutions"]
